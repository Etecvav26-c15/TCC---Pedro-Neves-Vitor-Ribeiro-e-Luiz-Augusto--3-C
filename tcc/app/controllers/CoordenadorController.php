...
class CoordenadorController {
    ...
    public function cadastrarMateria($dados)
{
    if (empty($dados['nome'])) {
        throw new Exception("Informe o nome da matéria.");
    }
    ...
    if ($existe) {
        throw new Exception("Esta matéria já está cadastrada.");
    }
...
public function cadastrarHorario($dados)
{
    ...
    return $this->db->query($sql, [
        $dados['professor_turma_materia_id'],
        $dados['dia_semana'],
...
{
    $sql = "INSERT INTO professor_turma_materia
            (professor_id, turma_id, materia_id, ano_letivo)
            VALUES (?, ?, ?, ?)";
